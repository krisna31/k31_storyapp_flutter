...
class _BuilderAddStoryState extends State<BuilderAddStory> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        _buildAddStoryGoogleMap(),
        _buildContentFormAddStory(),
      ],
    );
  }

  Positioned _buildContentFormAddStory() {
    return Positioned(
      top: 5,
      right: 16,
      left: 16,
  ...
  GoogleMap _buildAddStoryGoogleMap() {
    return GoogleMap(
      ...
      onLongPress: (latLng) {
        setState(() async {
          ...
          final marker = _buildMarker(latLng, address);
  ...
  Marker _buildMarker(LatLng latLng, String address) {
    return Marker(
      markerId: const MarkerId('Your Choosed Place'),
      position: latLng,
